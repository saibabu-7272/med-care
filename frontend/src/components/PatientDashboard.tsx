...
const PatientDashboard = () => {
  ...
  const fetchData = async () => {
    const response = await fetch("http://localhost:3000/taken-dates")
    const data = await response.json()
    
    const allDates = data.map(each => each.taken_date)
    if(response.ok){
      setTakenDates(new Set(allDates))
    }
  }
  ...
  const handleMarkTaken = async (date: string, imageFile?: File) => {
    const API = "http://localhost:3000/add-date"
    const options = {
      method : "POST",
      headers:{
      ...
      body : JSON.stringify({taken_date : date})
    }
